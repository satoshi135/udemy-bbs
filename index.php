...
members m, posts p WHERE m.id=p.member_id ORDER BY 
p.created DESC LIMIT ?,5');
...
<div id="wrap">
  ...
  <?php foreach ($posts as $post): ?>
    ...
    ENT_QUOTES)); ?>" width="48" height="48" alt="<?php 
    print(htmlspecialchars($post['name'], ENT_QUOTES)); ?>" />

    <p><?php print(htmlspecialchars($post['message'],
    ENT_QUOTES)); ?><span class="name">（<?php print(htmlspecialchars($post['name'],
    ENT_QUOTES)); ?>）</span>[<a href="index.php?res=
    <?php print(htmlspecialchars($post['id'], ENT_QUOTES)); ?>">Re</a>]</p>
    <p class="day"><a href="view.php?id="><?php print
    (htmlspecialchars($post['id'])); ?><?php print
...
返信元のメッセージ</a>
  ...
  [<a href="delete.php?id=<?php print(htmlspecialchars
  ($post['id'])); ?>"
